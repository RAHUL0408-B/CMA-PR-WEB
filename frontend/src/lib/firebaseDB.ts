 ...
 * Data lives at: /cma/{clients|reports|financials|projections|assumptions|loanSchedules|aiLogs}
 */
import {
  ref, get, set, push, update, remove, child, onValue, off,
  type DatabaseReference
} from 'firebase/database';
import { db, isRealtimeReady } from './firebase';
import { computeProjections, computeLoanSchedule } from './projectionEngine';

// ── Helpers ────────────────────────────────────────────────────
const ROOT = 'cma';
const dbRef = (path: string): DatabaseReference => ref(db!, `${ROOT}/${path}`);

/** Snapshot → array, injecting the key as `id` */
...
/** Generate a local ID (used as key when pushing) */
const uid = () => '_' + Math.random().toString(36).substring(2, 11);
...
export async function fbGetClients(): Promise<any[]> {
  const snap = await get(dbRef('clients'));
  const clients = snap2arr(snap);
  const repSnap = await get(dbRef('reports'));
...
export async function fbGetClient(id: string): Promise<any> {
  const snap = await get(dbRef(`clients/${id}`));
...
export async function fbCreateClient(data: any): Promise<any> {
  const newRef = push(dbRef('clients'));
  const newClient = {
    ...data,
...
export async function fbUpdateClient(id: string, data: any): Promise<any> {
  ...
  await update(dbRef(`clients/${id}`), updated);
  return { id, ...updated };
}

export async function fbDeleteClient(id: string): Promise<void> {
  await remove(dbRef(`clients/${id}`));
}
...
export async function fbGetReport(id: string): Promise<any> {
  const [repSnap, clientsSnap, finSnap, assSnap, projSnap, lsSnap] = await Promise.all([
    get(dbRef(`reports/${id}`)),
    get(dbRef('clients')),
    get(dbRef('financials')),
    get(dbRef(`assumptions/${id}`)),
    get(dbRef('projections')),
    get(dbRef(`loanSchedules/${id}`))
  ]);
...
export async function fbCreateReport(data: any): Promise<any> {
  const newRef = push(dbRef('reports'));
  const newReport = {
    ...data,
    status: 'DRAFT',
    createdAt: new Date().toISOString(),
...
export async function fbUpdateReport(id: string, data: any): Promise<any> {
  ...
  await update(dbRef(`reports/${id}`), updated);
  ...
  if (body.loanAmount || body.interestRate || body.loanTenure || body.moratoriumMonths !== undefined) {
    const snap = await get(dbRef(`reports/${id}`));
    ...
    await set(dbRef(`loanSchedules/${id}`), {
      reportId: id,
      loanAmount: Number(full.loanAmount || 0),
...
export async function fbGetFinancials(reportId: string): Promise<any[]> {
  const snap = await get(dbRef('financials'));
...
export async function fbUpsertFinancial(reportId: string, data: any): Promise<any> {
  // Check if a financial for this reportId+year already exists
  const snap = await get(dbRef('financials'));
  ...
  if (existing) {
    await update(dbRef(`financials/${existing.id}`), record);
    return { id: existing.id, ...record, createdAt: existing.createdAt };
  } else {
    const newRef = push(dbRef('financials'));
...
export async function fbDeleteFinancial(_reportId: string, yearId: string): Promise<void> {
  await remove(dbRef(`financials/${yearId}`));
}
...
export async function fbGetAssumptions(reportId: string): Promise<any> {
  const snap = await get(dbRef(`assumptions/${reportId}`));
...
export async function fbComputeProjections(reportId: string): Promise<any[]> {
  ...
  const ass = assSnap.exists() ? assSnap.val() : {
    ...
    depreciationRate: 10, taxRate: 25, capacityUtilization: '[70,80,85,90,95]', projectionYears: 5
  };

  const computed = computeProjections({
    basePL: safeParseJSON(baseYear.plData),
    baseBS: { assets: safeParseJSON(baseYear.bsAssets), liabilities: safeParseJSON(baseYear.bsLiabilities) },
    assumption: ass,
    report: {
  ...
  // Save projections — clear old ones first by overwriting at reportId key
  const projUpdates: Record<string, any> = {};
  computed.forEach((p: any) => {
    const k = push(child(ref(db!, `${ROOT}/projections`), '')).key || uid();
    projUpdates[k] = {
      reportId, year: p.year,
      plProjection: JSON.stringify(p.pl), bsProjection: JSON.stringify(p.bs),
      cfProjection: JSON.stringify(p.cf), ratios: JSON.stringify(p.ratios),
      dscr: p.ratios.dscr, createdAt: new Date().toISOString()
    };
  });
  await update(ref(db!, `${ROOT}/projections`), projUpdates);

  // Update report status
  await update(dbRef(`reports/${reportId}`), { status: 'IN_PROGRESS', updatedAt: new Date().toISOString() });
...
export async function fbGetProjections(reportId: string): Promise<any> {
  const [projSnap, lsSnap] = await Promise.all([
    get(dbRef('projections')),
    get(dbRef(`loanSchedules/${reportId}`))
  ]);
  const projections = snap2arr(projSnap)
    ...
    .map(p => ({
      id: p.id, reportId: p.reportId, year: p.year,
      plProjection: p.plProjection, bsProjection: p.bsProjection,
      cfProjection: p.cfProjection, ratios: p.ratios, dscr: p.dscr, createdAt: p.createdAt
    }));
