/**
 * CMA Pro AI - Financial Projection Engine (Frontend Copy)
...
export function computeProjections(params: {
  ...
  const capUtil: number[] = JSON.parse(assumption.capacityUtilization || "[70,80,85,90,95]");
  const years = assumption.projectionYears || 5;
  ...
  for (let i = 0; i < years; i++) {
    const growthFactor = 1 + (assumption.salesGrowthPct || 15) / 100;
    const val = capUtil[i];
    ...
    const pl: PLData = {
      ...
      rawMaterial: round(projectedSales * ((assumption.rawMaterialPct || 60) / 100)),
      salaryWages: round(prevPL.salaryWages * (1 + (assumption.salaryGrowthPct || 10) / 100)),
      powerFuel: round(projectedSales * ((assumption.powerExpensePct || 3) / 100)),
      manufacturingExp: round(prevPL.manufacturingExp * 1.05),
      adminExp: round(projectedSales * ((assumption.adminExpensePct || 5) / 100)),
      sellingExp: round(prevPL.sellingExp * 1.05),
      rent: round(prevPL.rent * 1.05),
      repairMaintenance: round(prevPL.repairMaintenance * 1.05),
      depreciation: round(((prevBS.assets.plantMachinery || 0) + (prevBS.assets.landBuilding || 0) + (prevBS.assets.vehicle || 0) + (prevBS.assets.furniture || 0)) * ((assumption.depreciationRate || 10) / 100)),
      interestExp: round((report.loanAmount || 0) * ((assumption.interestRate || 12) / 100) * (1 - i * 0.1)),
      taxExpense: 0
    };

    const plMetrics = computePLMetrics(pl);
    pl.taxExpense = round(Math.max(0, plMetrics.pbt) * ((assumption.taxRate || 25) / 100));

    const projBS: BSData = {
      assets: {
        landBuilding: round((prevBS.assets.landBuilding || 0) * (1 - (assumption.depreciationRate || 10) / 200)),
        plantMachinery: round((prevBS.assets.plantMachinery || 0) * (1 - (assumption.depreciationRate || 10) / 100)),
        furniture: round((prevBS.assets.furniture || 0) * 0.9),
        vehicle: round((prevBS.assets.vehicle || 0) * 0.85),
        investments: prevBS.assets.investments || 0,
        ...
        loansAdvances: prevBS.assets.loansAdvances || 0,
        otherCurrentAssets: prevBS.assets.otherCurrentAssets || 0
      },
      liabilities: {
        shareCapital: prevBS.liabilities.shareCapital || 0,
        ...
        unsecuredLoan: prevBS.liabilities.unsecuredLoan || 0,
        ccOdLimit: round(pl.grossSales * 0.08),
        tradeCreditors: round(pl.rawMaterial * 30 / 365),
        otherCurrentLiab: prevBS.liabilities.otherCurrentLiab || 0,
  ...
  for (let i = 0; i < projections.length; i++) {
    ...
    projectCashFlows.push(metrics.pat + pl.depreciation + pl.interestExp);
  }
